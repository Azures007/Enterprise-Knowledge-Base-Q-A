...
| 🔍 **混合检索** | 向量语义检索（text-embedding-v3）+ 关键词全文检索 双通道召回，默认 RRF（Reciprocal Rank Fusion）融合，兼容线性加权（0.7/0.3）；对专有名词/编号/代码片段类查询召回更准 |
...
| 🛠️ **工具调用** | Function Calling 支持：LLM 自主调用工具（列集合、检索知识库、查统计、时间/计算、天气），Agentic 检索按需跨集合查询；无来源时自动清理回答中的引用标注；工具循环带并行执行、轮数/调用次数/Token 三重上限、重复调用检测；写操作工具走「草稿-确认-执行」模式 |
...
内置检索评测脚本 [evaluate.py](evaluate.py)，用内置中文语料库（含专有名词/编号类难例）对比三种检索策略的召回质量，无需外部文档与数据库、不消耗 API Token：

```bash
python evaluate.py              # 默认 RRF，输出指标对比表
python evaluate.py --verbose    # 打印每个问题各策略的召回排名明细
python evaluate.py --mode weighted  # 单独评测指定融合模式
```

评测指标：**Recall@1/3/5、MRR、hit@1**。

参考结果（35 块语料 / 25 个问题，K=5）：

| 策略 | Recall@1 | Recall@3 | Recall@5 | MRR | hit@1 |
|------|---------|---------|---------|------|-------|
| 纯向量 | 0.40 | 0.64 | 0.76 | 0.541 | 0.40 |
| 混合（加权 0.7/0.3） | 0.64 | 0.92 | 0.92 | 0.773 | 0.64 |
| 混合（RRF） | 0.64 | **0.92** | **0.96** | **0.783** | 0.64 |

RRF 相对纯向量：**Recall@3 +43.8%，MRR +44.7%，hit@1 +60%**。混合检索（尤其 RRF）显著提升了专有名词/编号类查询的召回质量。
